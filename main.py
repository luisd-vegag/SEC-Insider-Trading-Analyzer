...
def create_trading_data(cik, start_date=None, end_date=None, days_range=0):
    tradingData = TradingData(
        cik, start_date, end_date, days_range)
    tradingData.acquired_disposed_by_insider()
    tradingData.stacked_bar_insider_ownership()
    tradingData.inside_trading_impact_plot()
    return tradingData


def create_form4_data(cik, start_date=None, end_date=None, days_range=0):
    form4Data = Form4(
        cik, start_date, end_date, days_range)
    return form4Data


def main(ciks, start_date=None, end_date=None, days_range=0, parallel_exc=2):
    ...
    with Pool(processes=parallel_exc) as pool:
        ...
        create_data_pool = partial(
            create_trading_data, start_date=start_date, end_date=end_date, days_range=days_range)
...
if __name__ == '__main__':
    ...
    ciks = ciks[1:2]
    main(ciks, start_date, end_date, days_range, parallel_exc=2)
    end_time = time.time()
